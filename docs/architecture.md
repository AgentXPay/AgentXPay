# AgentXPay — AI Agent 原生支付基础设施

> **让 AI Agent 在 Monad 区块链上自主发现、支付和消费 AI 服务，全程无需人工干预。**

AgentXPay 是构建在 [Monad](https://www.monad.xyz/) 高性能 EVM 区块链上的 **AI-to-AI 支付协议栈**，核心实现了 **x402 协议** —— 基于 HTTP 402 状态码的自动化链上支付标准。它为 AI Agent 经济体提供了从服务注册、自动支付、订阅管理到资金托管的完整闭环。
...
- **x402 自动支付协议**：AI Agent 发送 HTTP 请求 → 收到 402（`HTTP/1.1 402 Payment Required`） → 自动链上支付 → 携带交易凭证重试 → 获取服务响应，全程零人工干预
...
`AgentXPay`围绕 **x402 协议** 实现 `AI Agent`自主支付闭环，包括：

- **链上协议层**：5 个 `Solidity` 智能合约构成完整支付协议栈 —— 服务注册与发现（`ServiceRegistry`）、多模式支付（`PaymentManager`）、订阅管理（`SubscriptionManager`）、资金托管（`Escrow`）、Agent 智能钱包（`AgentWallet`）。
- **管控面层**：后端事件索引器实时监听链上支付与订阅事件并落库，前端管理面板提供服务管理、账单查询和数据可视化。
- **AI服务提供方**：AI服务提供方通过一行代码即可集成`AgentXPay`中间件即可将任意 `API` 转为付费接口，中间件负责返回 `402`、校验链上支付凭证并放行请求。
- **AI Agent消费方**：`AI Agent` 通过集成 `SDK` 或 `Skill` 工具包发起服务调用，`SDK` 内置 `x402` 感知能力，自动完成**"请求 → 402 → 支付 → 重试"**全流程。

```mermaid
graph TB
    subgraph "<b>AI Agent消费方</b>"
    ...
    subgraph "<b>管控面层</b>"
        IDX["@agentxpay/indexer<br/>后端事件索引器"]
        DB[("PostgreSQL<br/>事件存储")]
        FE["@agentxpay/frontend<br/>前端管理面板"]
        IDX --> DB
        FE --> DB
    end
...
x402 协议是 AgentXPay 的核心创新，基于 HTTP 402（Payment Required）状态码实现自动化链上支付：
...
基于 Solidity 0.8.20 + Foundry + OpenZeppelin 构建的链上协议：

```mermaid
graph LR
    subgraph "核心合约"
        SR["ServiceRegistry<br/>━━━━━━━━━━━<br/>· 服务注册与发现<br/>· 分类与过滤<br/>· 订阅计划管理"]
        PM["PaymentManager<br/>━━━━━━━━━━━<br/>· 按次付费 (payPerUse)<br/>· 批量付费 (batchPay)<br/>· 余额预充值/提取<br/>· 平台手续费"]
...
`@agentxpay/sdk` 是 Agent 与链上合约交互的核心客户端库：

```mermaid
classDiagram
    class AgentXPayClient {
        ...
        +discoverServices(filter) : Service[]
    }

    class ServicesModule {
        +registerService() : RegisterResult
        +discoverServices(filter) : Service[]
        +getServiceDetails(id) : Service
        +addSubscriptionPlan() : TxHash
        +getSubscriptionPlans(id) : Plan[]
    }

    class PaymentsModule {
        +payPerUse(serviceId, amount) : PaymentResult
        +batchPay(serviceIds, amount) : TxHash
        +deposit(amount) : TxHash
        +withdraw(amount) : TxHash
        +payFromBalance(serviceId) : TxHash
        +getProviderEarnings(addr) : BigInt
    }

    class SubscriptionsModule {
        +subscribe(serviceId, planId, amount) : SubResult
        +cancelSubscription(subId) : TxHash
        +renewSubscription(subId, amount) : TxHash
        +checkAccess(addr, serviceId) : Boolean
    }

    class EscrowModule {
        +createEscrow(serviceId, provider, deadline, desc, amount) : EscrowResult
        +releaseEscrow(id) : TxHash
        +disputeEscrow(id) : TxHash
        +refundEscrow(id) : TxHash
    }

    class WalletModule {
        +createWallet(dailyLimit) : WalletResult
        +getWallets(owner) : Address[]
    }

    AgentXPayClient --> ServicesModule
    AgentXPayClient --> PaymentsModule
    AgentXPayClient --> SubscriptionsModule
    AgentXPayClient --> EscrowModule
    AgentXPayClient --> WalletModule
```

**核心特性 — x402-aware fetch**：
...
const response = await client.fetch("https://provider.com/api/chat", {
  ...
  autoPayment: true,  // 自动支付
});
const data = await response.json();
```

### 3. x402 支付中间件 (middleware/)

`@agentxpay/middleware` 让任何 Express 服务一行代码变成付费 API：
...
flowchart TD
    ...
    SUB_OK{"订阅有效？"}
    SUB_PASS["注入 req.payment<br/>type: subscription<br/>→ next()"]
    ...
    PASS["注入 req.payment<br/>type: payment<br/>→ next()"]
...
| 组件 | 职责 |
|------|------|
| **`createPaymentGate`** | Express 中间件工厂，串联完整支付检查流程 |
| **`Facilitator`** | 路由 → 价格映射管理（支持精确匹配和 `*` 通配符） |
| **`PaymentVerifier`** | 链上交易验证 + LRU 缓存 + 防重放（txHash 只用一次） |

### 4. 事件索引器 (indexer/)

`@agentxpay/indexer` 轮询链上事件并存储到 PostgreSQL，提供 REST API 查询：

```mermaid
flowchart LR
    Chain["Monad 区块链<br/>PaymentManager<br/>SubscriptionManager"] -->|"轮询事件"| IDX["EventIndexer<br/>━━━━━━━━━━<br/>PaymentMade<br/>SubscriptionPaid"]
    IDX -->|"写入"| DB[("PostgreSQL<br/>━━━━━━━━━━<br/>payment_events<br/>indexer_state")]
    DB -->|"查询"| API["REST API<br/>━━━━━━━━━━<br/>/api/events<br/>/api/stats<br/>/api/indexer-status"]

    style Chain fill:#3B82F6,stroke:#2563EB,color:#fff
    style IDX fill:#14B8A6,stroke:#0D9488,color:#fff
...
基于 Next.js 14 + wagmi + RainbowKit 的管理面板：

| 页面 | 功能 |
|------|------|
| **Dashboard** | 概览统计（服务数、交易量、收入） |
| **Services** | 浏览和注册 AI 服务 |
| **Agent Wallet** | 创建和管理 Agent 智能钱包 |
| **Billing** | 查看支付历史和订阅状态 |
| **Playground** | 交互式试用 AI 服务 |

### 6. AI Agent Skill

`@agentxpay/skill` 让 LLM Agent 通过 Function Calling 自主使用支付能力：
...
| **前端** | Next.js 14, React 18, wagmi, viem, RainbowKit, Tailwind CSS, shadcn/ui, Recharts |
| **Skill** | TypeScript, OpenAI Function Calling / MCP / AgentSkills 兼容 |
| **构建** | pnpm workspaces, Turborepo, tsup, Vitest |
...
| 文档 | 说明 |
|------|------|
| [开发手册](./development-guide.md) | Agent 集成 SDK + Provider 集成 Middleware 完整指南 |
| [OpenClaw 集成手册](./openclaw-integration.md) | 将 AgentXPay Skill 接入 OpenClaw 平台 |
